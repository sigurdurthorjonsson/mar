...
#' @return dataframe
#' @export
mar_tables <- function(con, schema) {

  d <- tbl_mar(con, "sys.all_tables")
  #  v <- tbl_mar(con,'sys.all_views') %>%
  ...
  d %>%
    ...
    dplyr::left_join(tbl_mar(con,'sys.all_tab_comments')) %>%
...
#' @return dataframe
#' @export
mar_fields <- function(con, table) {

  x <- strsplit(table,'\\.') %>% unlist()

  #  tbl_mar(con,'all_tab_columns') %>%
