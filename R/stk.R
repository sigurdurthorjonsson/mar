...
#' stk_mapdeck
#'
#' @param con Oracle connectoin
#' @param MID mobileid in stk
#'
#' @export
#'
...
#' @param correct Correct mid
#' @param vidmatch Match vessels
#' @param classify Classify the mobileid
#'
#' @export
#'
stk_mobile <- function(con, correct = FALSE, vidmatch = FALSE, classify = FALSE) {
  ...
  if( correct ) {
    ...
    q <-
      ...
      dplyr::mutate(localid = dplyr::case_when(mid == 101069 ~ "1275",    # Jon Vidalí-in
  ...
  if( classify ) {
    ...
    q <-
      ...
      dplyr::mutate(mmsi.mid = dplyr::case_when(as.integer(stringr::str_sub(mmsi, 1, 1)) %in% c("2", "3", "4", "5", "6", "7") ~ stringr::str_sub(mmsi, 1, 3),
                                  stringr::str_sub(mmsi, 1, 2) %in% c("98", "99") ~ stringr::str_sub(mmsi, 3, 5),
                                  TRUE ~ NA_character_))
...
stk_mobile_icelandic <- function(con, correct = FALSE, vidmatch = FALSE, classify = FALSE) {
  ...
  if( correct ) {
    ...
    q <-
      ...
      #        mt states IMO 9039779, which is old vid 2173
      #
      dplyr::mutate(localid = dplyr::case_when(mid == 100013 ~ "6643",    # 2020-03-10: Gimli, yfirskrifa skipnr 1156 sem datt af skra 2007                                               mid == 101069 ~ "1275",    # Jon Vidalí-in
                                               mid == 101078 ~ "2549",    # Thor HF-4 NOTE: DUBIOUS CORRECTION,
...
# #RUN: 2020-03-10
# d <-
#   stk_mobile_icelandic(con, TRUE, TRUE, TRUE) %>%
#   left_join(stk_trail(con) %>%
#               group_by(mid) %>%
#               summarise(n.vms = n())) %>%
#   collect(n = Inf)
