...
## FROM (fiskar.lengdir) "qmnezksmoa"
```
...
## FROM (fiskar.lengdir) "cjhcimlohp") "giwfoxkwbl") "fpacxhijil"
...
## FROM (fiskar.lengdir) "ckpkapwcca") "iztdwhashp"
## 
## INNER JOIN
## 
...
## FROM (fiskar.stodvar) "dkrhhidvyk") "igwcklpgds"
## WHERE ("dags" > TO_DATE('01.01.1986', 'dd.mm.yyyy'))) "mazvlyhlrl") "jtzqvigicq"
## 
## USING ("synis_id")) "tfviueamst") "lojxkqhwdt"
## UNION ALL
...
## FROM (fiskar.leidr_lengdir) "jtpvvemtjh") "eafrgprnda") "kmrtirlkoo"
## 
## INNER JOIN
## 
## (SELECT "synis_id" AS "synis_id"
...
## FROM (fiskar.leidr_stodvar) "jsubfurxhc") "zqiouaaevs"
## WHERE ("dags" < TO_DATE('01.01.1986', 'dd.mm.yyyy') AND "dags" > TO_DATE('1910', 'yyyy'))) "qnerqgeqjk") "nhoudmlsmc"
## 
## USING ("synis_id")) "uoketmvlsc"
## WHERE (NOT(("synis_id" IN (133095.0, 57070.0, 133401.0, 37559.0, 112980.0, 112984.0, 112987.0, 112991.0, 112995.0, 112998.0, 112999.0, 128268.0, 129166.0, 129168.0, 140153.0, 140155.0, 129370.0, 129170.0, 128765.0, 129098.0, 119798.0, 128890.0, 129146.0, 128586.0, 128898.0, 128902.0, 123916.0, 128392.0, 116665.0, 115948.0, 115967.0))))) "ulxcmzggpv") "vecndrbbea") "tysbysftrx"
## WHERE ("synis_id" = 48489.0)) "nvbblqzshi"
## GROUP BY "tegund") "suyxmrxaco"
## ORDER BY "fjoldi"
```
