...
## FROM (fiskar.lengdir) "sfhftimcei"
```
...
## FROM (fiskar.lengdir) "lddqtobkgx") "mquinboylg") "eadfbeoprz"
...
## FROM (fiskar.lengdir) "uuofqjhibi") "cnsidpuglb"
## 
## INNER JOIN
## 
...
## FROM (fiskar.stodvar) "lfvqpphbwp") "vohozjmmqk"
## WHERE ("dags" > TO_DATE('01.01.1986', 'dd.mm.yyyy'))) "nuwgnmwivj") "vbaeeutoql"
## 
## USING ("synis_id")) "zyvziezgsz") "rqpbxlrqnc"
## UNION ALL
...
## FROM (fiskar.leidr_lengdir) "wvbxwjevjy") "hunhspjsxf") "xgozchrufp"
## 
## INNER JOIN
## 
## (SELECT "synis_id" AS "synis_id"
...
## FROM (fiskar.leidr_stodvar) "leqkcrssyv") "hgevhnmgit"
## WHERE ("dags" < TO_DATE('01.01.1986', 'dd.mm.yyyy') AND "dags" > TO_DATE('1910', 'yyyy'))) "gwcepjpyag") "mxojrtkjvf"
## 
## USING ("synis_id")) "jtmfnqdyql"
## WHERE (NOT(("synis_id" IN (133095.0, 57070.0, 133401.0, 37559.0, 112980.0, 112984.0, 112987.0, 112991.0, 112995.0, 112998.0, 112999.0, 128268.0, 129166.0, 129168.0, 140153.0, 140155.0, 129370.0, 129170.0, 128765.0, 129098.0, 119798.0, 128890.0, 129146.0, 128586.0, 128898.0, 128902.0, 123916.0, 128392.0, 116665.0, 115948.0, 115967.0))))) "rkmueaudxg") "vfnorfcxwa") "iptqdbozlz"
## WHERE ("synis_id" = 48489.0)) "lvtytfqstq"
## GROUP BY "tegund") "cycwkzunau"
## ORDER BY "fjoldi"
```
...
##  data.table    1.9.7       2016-08-08 local                        
##  DBI           0.5-1       2016-09-10 cran (@0.5-1)                
##  devtools      1.11.1.9000 2016-06-07 github (hadley/devtools)     
##  digest        0.6.10      2016-08-02 cran (@0.6.10)               
##  dplyr       * 0.5.0.9000  2016-11-12 github (hadley/dplyr)        
##  dplyrOracle * 0.0.1       2016-11-11 github (fishvice/dplyrOracle)
##  evaluate      0.9         2016-04-29 CRAN (R 3.2.3)               
##  formatR       1.4         2016-05-09 cran (@1.4)                  
##  ggplot2     * 2.1.0.9000  2016-08-04 github (hadley/ggplot2)      
##  gisland       0.0.05      2016-09-02 local                        
...
##  mar         * 0.0.3.9000  2016-11-12 local                        
##  memoise       1.0.0       2016-01-29 CRAN (R 3.2.3)               
##  munsell       0.4.3       2016-02-13 CRAN (R 3.2.3)               
##  plyr          1.8.4       2016-06-08 cran (@1.8.4)                
##  purrr       * 0.2.2.9000  2016-08-13 github (hadley/purrr)        
...
##  sp          * 1.2-3       2016-04-14 CRAN (R 3.2.3)               
##  stringi       1.1.1       2016-05-27 CRAN (R 3.2.3)               
##  stringr       1.1.0       2016-08-19 CRAN (R 3.3.0)               
##  tibble      * 1.2-12      2016-11-12 github (hadley/tibble)       
##  tidyr       * 0.6.0.9000  2016-08-13 github (hadley/tidyr)        
